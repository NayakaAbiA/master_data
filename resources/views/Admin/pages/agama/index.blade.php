                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="buttons">
                                        ...
                                        <a class="btn icon btn-primary" href="{{ route('admin.agama.edit',$item['id'] )}}"><i class="bi bi-pencil-square"></i></a>
                                        <form action="{{ route('admin.agama.destroy',$item['id'] )}}" method="POST" style="display:inline;">
                                            @csrf
                                            @method('DELETE')
            
                                            <button class ="btn icon btn-primary" type="submit" onclick="return confirm('Apakah anda yakin ingin menghapus {{ $item['nama_agama']}} ?')"><i class="bi bi-trash3"></i></button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
