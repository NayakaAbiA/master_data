            ...
            <table class="table table-striped" id="table1">
                ...
                @foreach ($transportasi as $item)
                    <tr>
                        ...
                        <td>{{ $item['alat_transport']}}</td>
                        <td>
                            <div class="buttons">
                                <!-- parameter diambil berdasarkan route yang ada di web.php -->
                                <a class="btn icon btn-primary" href="{{ route('admin.transportasi.edit',  $item['id'] )}}"><i class="bi bi-pencil-square"></i></a>
                                <form action="{{ route('admin.transportasi.destroy',  $item['id'] )}}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
    
                                    <button class ="btn icon btn-primary" type="submit" onclick="return confirm('Apakah anda yakin ingin menghapus {{ $item['alat_transport'] }} ?')"><i class="bi bi-trash3"></i></button>
                                </form>
                            </div>
                        </td>
                    </tr>
